...
const createJwtToken = (body, options) =>
  jwt.sign(body, process.env.JWT_PRIVATE_KEY, options);
...
const isHuman_C = (input) => {
  ...
  axios
    .post(
      `https://www.google.com/recaptcha/api/siteverify?secret=${secret}&response=${input.token}`
    )
