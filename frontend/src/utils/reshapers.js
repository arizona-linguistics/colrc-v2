...
export function audioReshape(jsonData) {
   // Original 
   //  let json = JSON.parse(JSON.stringify(jsonData))
   //  console.log("our json in audioReshape is ", json)
   //  while (json.length) {
   //    let i = 0
   //    let j = 0
         ...
         json[i]["sourcefiles"].push(
            {
               ...
               path: null,
               type: "text",
               key: j // make it unique
            }
         );
         j++;

      } else {
         while (j < json[i]["text"]["texts_textfiles"].length) {
            // Convert metadata into series of fields and values
            let metadata = []
            if (json[i]["text"]["texts_textfiles"][j]["textfilemetadata"].length > 0) {
               Object.entries(json[i]["text"]["texts_textfiles"][j]["textfilemetadata"][0].metadata).forEach(
                  ([field, value]) => metadata.push({ field: field, value: value }))
            }

            json[i]["sourcefiles"].push(
               {
                  src: json[i]["text"]["texts_textfiles"][j].src,
                  title: json[i]["text"]["texts_textfiles"][j].resType + " pdf",
                  fileType: json[i]["text"]["texts_textfiles"][j].fileType,
                  metadata: metadata,
                  msType: json[i]["text"]["texts_textfiles"][j].msType,
                  path: json[i]["text"]["texts_textfiles"][j].textfile_with_path,
                  type: "text",
                  key: k
               }
            );
            if (json[i]["text"]["texts_textfiles"][j]["textimages"].length > 0) {
               k++;
               // for each textfile, build the query string so that imageviewer can derive the images from it.
               let l = 0;
               json[i]["text"]["texts_textfiles"][j]["imagequerystring"] = '';
               while (l < json[i]["text"]["texts_textfiles"][j]["textimages"].length) {
                  json[i]["text"]["texts_textfiles"][j]["imagequerystring"] = json[i]["text"]["texts_textfiles"][j]["imagequerystring"] + '&images=' + json[i]["text"]["texts_textfiles"][j]["textimages"][l]["textimage_with_path"];
                  l++;
               }
               //console.log(json[i]["texts_textfiles"][j]["imagequerystring"]);
               ...
               json[i]["sourcefiles"].push(
                  {
                     src: json[i]["text"]["texts_textfiles"][j]["imagequerystring"],
                     title: json[i]["text"]["texts_textfiles"][j].resType + " image files",
                     fileType: json[i]["text"]["texts_textfiles"][j].fileType,
                     type: "textimages",
                     key: k
                  }
               ...
               //SplitView needs to know if the imagefiles are for handwritten
               //or typed fieldnotes.  Here's where we tell it.
               if (json[i]["text"]["texts_textfiles"][j].msType === "Handwritten") {
                  handImages = json[i]["text"]["texts_textfiles"][j]["imagequerystring"];
               }
               if (json[i]["text"]["texts_textfiles"][j].msType === "Typed") {
                  typedImages = json[i]["text"]["texts_textfiles"][j]["imagequerystring"];
               }
            }
            j++; k++;
         }
