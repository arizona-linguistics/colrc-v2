  ...
  function createSpans(str) {
     ...
     for (let i = 0; i < decorations.length; i++)
     {
       ...
       if (decorations[i].type === "bold") {
         children.push(<span key={i} style={{ fontWeight: 'bold' }}>{ss}</span>);
       }
       else if (decorations[i].type === "underline") {
         children.push(<span key={i} style={{ textDecoration: 'underline' }}>{ss}</span>);
       }
       else if (decorations[i].type === "superscript") {
        children.push(<span key={i} style={{ fontSize: '70%', verticalAlign: 'super' }}>{ss}</span>);
       }
       else {
