...
import {
  getElicitationSetByIdQuery,
  updateElicitationSetMutation,
...
let updateElicitationSchema = Yup.object().shape({
  language: Yup.string().required("a transcription language is required"),
  prompt: Yup.string().required("an elicitation prompt is required"),
  editnote: Yup.string().required("an edit note is required"),
  speaker: Yup.string().required("a speaker is required"),
  transcription: Yup.string().required(
    "an elicitation transcription is required"
  ),
});
...
function EditElicitation() {
  ...
  } = useQuery(getElicitationSetByIdQuery, {
    client: client,
    variables: { id: id },
  });
  ...
  async function onFormSubmit(values, setSubmitting) {
    try {
      ...
      if (result.error) {
        ...
        broadCastSuccess(
          `elicitation set ${values.transcription} successfully edited!`
        );
        setSubmitting(false);
        setHasUpdated(true);
      }
    ...
    <>
        ...
        <Grid.Row>
          <Grid.Column textAlign="center" width={12}>
            ...
            <Message>
              ...
              elicitation sets. The elements whose labels are outlined may be
              blank.
            </Message>
          </Grid.Column>
        </Grid.Row>
      ...
      <Formik
        initialValues={{
          ...
          language: elicitationData.elicitationsets_by_pk.language
            ? elicitationData.elicitationsets_by_pk.language
            : "",
          prompt: elicitationData.elicitationsets_by_pk.prompt
            ? elicitationData.elicitationsets_by_pk.prompt
            : "",
          speaker: elicitationData.elicitationsets_by_pk.speaker
            ? elicitationData.elicitationsets_by_pk.speaker
            : "",
          transcription: elicitationData.elicitationsets_by_pk.transcription
            ? elicitationData.elicitationsets_by_pk.transcription
            : "",
          editnote: elicitationData.elicitationsets_by_pk.editnote
            ? elicitationData.elicitationsets_by_pk.editnote
            : "",
        }}
            ...
            <Grid centered>
              ...
              <Grid.Row>
                ...
                <Grid.Column width={10}>
                  ...
                  />
                  {errors.prompt && touched.prompt && (
                    <div className="input-feedback">{errors.prompt}</div>
                  )}
                </Grid.Column>
              </Grid.Row>
              <Grid.Row>
                ...
                <Grid.Column width={10}>
                  ...
                  />
                  {errors.transcription && touched.transcription && (
                    <div className="input-feedback">{errors.transcription}</div>
                  )}
                </Grid.Column>
              </Grid.Row>
              <Grid.Row>
                ...
                <Grid.Column width={10}>
                  ...
                  />
                  {errors.speaker && touched.speaker && (
                    <div className="input-feedback">{errors.speaker}</div>
                  )}
                </Grid.Column>
              </Grid.Row>
              <Grid.Row>
