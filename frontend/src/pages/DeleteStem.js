...
function DeleteStem() {
  ...
  let {
    ...
    error: stemError,
    data: stemData,
  } = useQuery(getStemByIdQuery, { client: client, variables: { id: id } });
  let {
    ...
    <>
      <Grid centered>
        <Grid.Row>
          <Grid.Column textAlign="center" width={12}>
            ...
            <Message>
              Submitting this form removes the affix from this application.
              Removed elements can only be re-instated by a manager.
            </Message>
          </Grid.Column>
        </Grid.Row>
      </Grid>
      <Formik
        initialValues={{
          ...
          salish: stemData.stems_by_pk.salish
            ? stemData.stems_by_pk.salish
            : "",
          english: stemData.stems_by_pk.english,
          doak: stemData.stems_by_pk.doak ? stemData.stems_by_pk.doak : "",
          reichard: stemData.stems_by_pk.reichard
            ? stemData.stems_by_pk.page
            : "",
          editnote: stemData.stems_by_pk.editnote
            ? stemData.stems_by_pk.editnote
            : "",
        }}
