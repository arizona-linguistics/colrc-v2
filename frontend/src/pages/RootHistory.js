...
function RootHistory(props) {
    ...
    function MakeRows(){
        ...
        data.audit_logged_actions.map(elem => {
          return (
              { id: elem.event_id, 
                action: elem.action, 
        ...
        return (
          <tr>
            {rows.map(row => (
              <td key={row.id}>ID: {row.id}, Action: {row.action}, UserId: {row.userId}, Editnote: {row.editnote}, Root: {row.root}, Nicodemus: {row.english}, English: {row.english}, Salish: {row.salish}</td>
            ))}
          </tr>
        )
      } 
