...
import { getUserByIdQuery, getRolesQuery, deleteUserMutation } from '../queries/queries'
import { Button, Input, Label, Grid, Header, Message, Dropdown } from 'semantic-ui-react';
...
function DeleteUser(props) {
    ...
    async function onFormSubmit (values, setSubmitting) {
        ...
        if (result.error) {
            ...
            broadCastSuccess(`User ${values.username} successfully removed!`)
            setSubmitting(false)
            setHasUpdated(true)
        }
        } catch (error) {
    ...
    <Grid centered>
        <Grid.Row>
            <Grid.Column textAlign="center" width={12}>
                <Header as="h2">Remove User</Header>
                <Message>Use this form to remove a user account</Message>
            </Grid.Column>
        </Grid.Row>
    </Grid>
    <Formik 
