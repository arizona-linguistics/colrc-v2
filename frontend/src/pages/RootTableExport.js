...
function Table({
  ...
  console.log(
    "my location.state.globalSearch is ",
  ...
  function getExportFileName({ fileType, all }) {
    let fileName = "";
    fileName =
      all === true ? "roots_all_rows_all_cols" : "roots_all_rows_sel_cols";
    return fileName;
  }
    ...
    <>
      ...
      {!show ? (
        <>
          <Message>
            Export all rows or{" "}
            <Link to={{ pathname: "/roots" }}>export visible rows</Link>
          </Message>
          <Message>
            This page provides access to all data in the roots table, so page
            loading and export may be slower than expected. Export to pdf, in
            particular, will take several seconds to complete.
          </Message>
          <Grid columns={2}>
            <Grid.Column>
              <Segment>
